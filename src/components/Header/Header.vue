<template>
  <nav class="fixed-top navbar navbar-expand-lg navbar-light bg-dark">
    ...
    <div class="collapse navbar-collapse" id="navbarNav">
      <ul class="navbar-nav">
        <li class="nav-item">
          <router-link to="/panier" class="nav-link text-light " href="#">Mon panier ({{qts}}) <i
              class="fas fa-shopping-bag"></i></router-link>
        </li>
        <li class="nav-item">
          <router-link class="nav-link text-light " to="/">Accueil <i class="fas fa-store-alt"></i></router-link>
        </li>
        <li class="nav-item">
          <router-link class="nav-link text-light " to="/">A propos du site <i class="fas fa-info-circle"></i>
          </router-link>
        </li>
      </ul>
    </div>
      ...
      calculPanier: function () {
        ...
        if (localStorage.length > 0) {
          ...
          for (i; i < localStorage.length; i++) {
            ...
            this.qts += this.panier[i].quantite
